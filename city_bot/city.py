...
async def situation(msg: Message, state: FSMContext):
    ...
    text = msg.text
    user_answer = text
    if user_answer.lower() == 'я не знаю' or user_answer.lower() == 'не знаю':
        await msg.answer('Вы не знаете больше городов на эту букву? Значит я победил!')
        await msg.answer('/play')
    elif user_answer.lower()[0] != bot_answer[-1] and user_answer.lower()[0] != bot_answer[-2] and user_answer.lower()[0] != bot_answer[-3]:
        if bot_answer[-1] == 'ё' or bot_answer[-1] == 'й' or bot_answer[-1] == 'ъ' or bot_answer[-1] == 'ы' or bot_answer[-1] == 'ь':
            if bot_answer[-2] == 'ё' or bot_answer[-2] == 'й' or bot_answer[-2] == 'ъ' or bot_answer[-2] == 'ы' or bot_answer[-2] == 'ь':
                await msg.answer( f'Неправильно. город должен начинаться с буквы {bot_answer[-3]}')
            else:
                await msg.answer(f'Неправильно. город должен начинаться с буквы {bot_answer[-2]}')
        else:
            await msg.answer( f'Неправильно. город должен начинаться с буквы {bot_answer[-1]}')
    elif user_answer not in cities1:
        await msg.answer( 'Такого города в России не существует! Попробуйте снова')
    elif user_answer not in cities2:
        await msg.answer( 'Такой город уже называли')
    else:
        await msg.answer('Верно!')
        if user_answer[-1] == 'ё' or user_answer[-1] == 'й' or user_answer[-1] == 'ъ' or user_answer[-1] == 'ы' or user_answer[-1] == 'ь':
            if user_answer[-2] == 'ё' or user_answer[-2] == 'й' or user_answer[-2] == 'ъ' or user_answer[-2] == 'ы' or user_answer[-2] == 'ь':
                await msg.answer(f'Мне на букву {user_answer[-3]}')
            else:
                await msg.answer( f'Мне на букву {user_answer[-2]}')
        else:
            await msg.answer( f'Мне на букву {user_answer[-1]}')
        cities2.remove(user_answer)

        for candidate in cities2:
            if user_answer[-1] == 'ё' or user_answer[-1] == 'й' or user_answer[-1] == 'ъ' or user_answer[-1] == 'ы' or user_answer[-1] == 'ь':
                if user_answer[-2] == 'ё' or user_answer[-2] == 'й' or user_answer[-2] == 'ъ' or user_answer[-2] == 'ы' or user_answer[-2] == 'ь':
                    if candidate.lower()[0] == user_answer[-3]:
                        bot_answer = candidate
                        cities2.remove(candidate)
                        await msg.answer(f'Мой город: {candidate}')
                        if candidate[-1] == 'ё' or candidate[-1] == 'й' or candidate[-1] == 'ъ' or candidate[-1] == 'ы' or candidate[-1] == 'ь':
                            if candidate[-2] == 'ё' or candidate[-2] == 'й' or candidate[-2] == 'ъ' or candidate[-2] == 'ы' or candidate[-2] == 'ь':
                                await msg.answer( f'Вам на букву {candidate[-3]}')
                            else:
                                await msg.answer( f'Вам на букву {candidate[-2]}')
                        else:
                            await msg.answer(f'Вам на букву {candidate[-1]}')
                        break
                else:
                    if candidate.lower()[0] == user_answer[-2]:
                        bot_answer = candidate
                        cities2.remove(candidate)
                        await bot.send_message(msg.chat.id, f'Мой город: {candidate}')
                        if candidate[-1] == 'ё' or candidate[-1] == 'й' or candidate[-1] == 'ъ' or candidate[-1] == 'ы' or candidate[-1] == 'ь':
                            if candidate[-2] == 'ё' or candidate[-2] == 'й' or candidate[-2] == 'ъ' or candidate[-2] == 'ы' or candidate[-2] == 'ь':
                                await msg.answer(f'Вам на букву {candidate[-3]}')
                            else:
            ...
            await msg.answer(f'Я больше не знаю городов на букву {user_answer[-1]}. Сдаюсь, вы победили!')
            await state.clear()
            await msg.answer('/play')
...
async def command_start_handler(msg: Message):
    ...
    await msg.answer( "Нажмите /play")


@dp.message(Command("play"))
async def play(msg: Message, state: FSMContext):
    await msg.answer(f'Мой город: {bot_answer}')
    if bot_answer[-1] == 'ё' or bot_answer[-1] == 'й' or bot_answer[-1] == 'ъ' or bot_answer[-1] == 'ы' or bot_answer[-1] == 'ь':
        if bot_answer[-2] == 'ё' or bot_answer[-2] == 'й' or bot_answer[-2] == 'ъ' or bot_answer[-2] == 'ы' or bot_answer[-2] == 'ь':
            await msg.answer(f'Вам на букву {bot_answer[-3]}')
        else:
            await msg.answer(f'Вам на букву {bot_answer[-2]}')
    else:
        await msg.answer(f'Вам на букву {bot_answer[-1]}')
